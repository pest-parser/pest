...
mod test {
    ...
    fn test_restoring_on_err() {
        ...
        impl Parser<Rule> for StackUseError {
            fn parse<'i>(
                rule: Rule,
                input: &'i str,
                ...
                pub fn expr(state: Box<ParserState<Rule>>) -> ParseResult<Box<ParserState<Rule>>> {
                    state.rule(Rule::expr, |state| {
                        state
                            .restore_on_err(|state| will_fail(state))
                            .or_else(|state| state.restore_on_err(|state| stack_use(state)))
                    })
                }

                ::state(input, |state| match rule {
                    Rule::stack_use => stack_use(state),
                    Rule::will_fail => will_fail(state),
                    Rule::expr => expr(state),
                })
            }
        }

        assert!(StackUseError::parse(Rule::expr, "AABAA").is_ok());
    }
}
