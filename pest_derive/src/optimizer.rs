    ...
    fn unroll_loop_exact() {
        let rules = vec![
            Rule {
                name: "rule",
                ty: RuleType::Atomic,
                expr: Expr::RepExact(Box::new(Expr::Ident("a")), 3)
            },
        ];
        let unrolled = vec![
            Rule {
    ...
    fn unroll_loop_max() {
        let rules = vec![
            Rule {
                name: "rule",
                ty: RuleType::Atomic,
                expr: Expr::RepMax(Box::new(Expr::Str(&"a")), 3)
            },
        ];
        let unrolled = vec![
            Rule {
                ...
                ty: RuleType::Atomic,
                expr: Expr::Seq(
                    Box::new(Expr::Opt(Box::new(Expr::Str(&"a")))),
                    Box::new(Expr::Seq(
                        Box::new(Expr::Opt(Box::new(Expr::Str(&"a")))),
                        Box::new(Expr::Opt(Box::new(Expr::Str(&"a"))))
                    ))
    ...
    fn unroll_loop_min() {
        let rules = vec![
            Rule {
                name: "rule",
                ty: RuleType::Atomic,
                expr: Expr::RepMin(Box::new(Expr::Str(&"a")), 2)
            },
        ];
        let unrolled = vec![
            Rule {
                ...
                expr: Expr::Seq(
                    ...
                    Box::new(Expr::Seq(
                        Box::new(Expr::Str(&"a")),
                        Box::new(Expr::Rep(Box::new(Expr::Str(&"a"))))
                    ))
                )
            },
        ];
    ...
    fn unroll_loop_min_max() {
        let rules = vec![
            Rule {
                name: "rule",
                ty: RuleType::Atomic,
                expr: Expr::RepMinMax(Box::new(Expr::Str(&"a")), 2, 3)
            },
        ];
        let unrolled = vec![
            Rule {
