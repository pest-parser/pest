...
impl Vm {
    ...
    pub fn parse<'i>(
        ...
        if let Some(rule) = self.rules.get(rule) {
            let result = pest::state(input, |mut state, pos| {
                self.parse_rule(input, rule, pos, &mut state)
            });
    ...
    fn parse_rule<'a, 'i>(
        ...
        let is_atomic = (rule.ty == RuleType::Atomic || rule.ty == RuleType::CompoundAtomic)
            || &rule.name == "whitespace" || &rule.name == "comment";

        match rule.ty {
            RuleType::Normal => state.rule(&rule.name, pos, |state, pos| {
                self.parse_expr(input, &rule.expr, is_atomic, pos, state)
            }),
            RuleType::Silent => self.parse_expr(input, &rule.expr, is_atomic, pos, state),
